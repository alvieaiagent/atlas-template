...
export const CRAWL_BUDGET_MODES = [
  {
    value: "Conservative",
    recommended: true,
    cadence: "Friday 9:00am HKT · P0 watchlist only",
    scope: "Max 3 YouTube videos/channel, 6 IG posts/creator, 10 X/Threads results/lane.",
  },
  {
    value: "Balanced",
    recommended: false,
    cadence: "Tuesday + Friday 9:00am HKT",
    scope: "P0 plus selected P1 watchlist with slightly higher source limits.",
  },
  {
    value: "Manual Only",
    recommended: false,
    cadence: "No automatic crawl",
    scope: "Force refresh only. Safest while Alvie is not posting publicly yet.",
  },
  {
    value: "Aggressive",
    recommended: false,
    cadence: "Daily · not recommended",
    scope: "Only consider when Atlas is actively supporting public publishing or monetization.",
  },
] as const;
...
export type DailySummary = JaniceSummary & {
  id: string;
  dateHkt: string;
  learningArea: LearningAreaLabel;
  isFallback: boolean;
};

export type WatchlistItem = {
  name: string;
  source: Source | "website" | "other";
  urlOrHandle: string;
  priority: WatchlistPriority;
  learningArea: LearningAreaLabel;
  notes: string;
  lastRefreshed: string;
  nextSuggestedRefresh: string;
  persisted: boolean;
};

export function getLearningArea(slug: string): (typeof LEARNING_AREAS)[number] | undefined {
  return LEARNING_AREAS.find((area) => area.slug === slug);
}
...
export const FALLBACK_DAILY_SUMMARIES: DailySummary[] = LEARNING_AREAS.map((area, index) => ({
  ...
  isFallback: true,
  executiveSummary:
    "No generated daily summary has been saved yet. This card is a V2 structure preview only, so do not treat it as market intelligence.",
  keyPoints: [
    "Atlas needs real crawled links, notes, or selected watchlist items before Janice can brief this lane.",
    "Use Quick Capture or Force Refresh first, then save the useful outputs into Knowledge Bank.",
  ],
  ...
  implicationForAlvie:
    "Use this lane to turn reading into career, business, CityU, AI, content, or partnership POV only after sources are captured.",
...
export const FALLBACK_WATCHLIST: WatchlistItem[] = [
  {
    name: "P0 AI platform leaders",
    source: "x",
    urlOrHandle: "@example-ai-platform-list",
    priority: "P0 Weekly",
    learningArea: "AI Market Movement",
    notes: "Typed fallback: replace with real high-signal X accounts or lists.",
    lastRefreshed: "Not connected",
    nextSuggestedRefresh: "Friday 9:00am HKT",
    persisted: false,
  },
  {
    name: "Priority YouTube learning channels",
    source: "youtube",
    urlOrHandle: "Paste specific channel/video URLs",
    priority: "Manual Only",
    learningArea: "Growth Case Studies",
    notes: "YouTube is supported first through Quick Capture; avoid broad channel crawling until needed.",
    lastRefreshed: "Manual capture only",
    nextSuggestedRefresh: "When Alvie finds a useful video",
    persisted: false,
  },
  {
    name: "Instagram creator references",
    source: "ig",
    urlOrHandle: "@manual-watchlist",
    priority: "P0 Weekly",
    learningArea: "Creator Economy",
    notes: "Conservative scope: latest 6 posts/reels per P0 creator.",
    lastRefreshed: "Not connected",
    nextSuggestedRefresh: "Friday 9:00am HKT",
    persisted: false,
  },
