  ...
  return (
    <main className="flex min-w-0 flex-1 flex-col gap-5 p-4 md:p-6">
      ...
      <section className="rounded-xl border border-blue-100 bg-white p-5 shadow-sm">
        <div className="mb-4">
          <h2 className="text-lg font-bold text-slate-950">Crawl Budget Mode</h2>
          <p className="mt-1 text-sm leading-6 text-slate-600">
            <strong className="text-slate-950">Conservative is recommended.</strong> Daily cron is disabled because Alvie is not publishing publicly yet and daily Apify crawling would waste credits.
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {CRAWL_BUDGET_MODES.map((mode) => (
            <article key={mode.value} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-center justify-between gap-2">
                <h3 className="font-bold text-slate-950">{mode.value}</h3>
                {mode.recommended ? <span className="rounded-full bg-blue-600 px-2 py-1 text-[11px] font-bold text-white">Recommended</span> : null}
              </div>
              <p className="mt-2 text-sm font-bold text-slate-700">{mode.cadence}</p>
              <p className="mt-1 text-xs leading-5 text-slate-500">{mode.scope}</p>
            </article>
          ))}
        </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        <div className="mb-4">
          <h2 className="text-base font-semibold text-slate-950">{copy.dataSources}</h2>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <form
          ...
          className="mb-5 grid gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 md:grid-cols-[1fr_1fr_1fr_88px_96px]"
        >
